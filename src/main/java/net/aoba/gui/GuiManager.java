...
public class GuiManager implements KeyDownListener, TickListener, Render2DListener, MouseClickListener,
	...
	public void Initialize() {
		...
		boolean hasModules = !Aoba.getInstance().moduleManager.modules.isEmpty();
		if (hasModules) {
		...
		ArrayList<HudWindow> huds = Lists.newArrayList();
		if (hasModules)
			huds.add(moduleSelector);
		huds.addAll(List.of(armorHud, radarHud, timeHud, dayHud));
		if (hasModules)
			huds.add(moduleArrayListHud);
		huds.addAll(List.of(watermarkHud, coordsHud, netherCoordsHud, fpsHud, pingHud, speedHud, inputHud));
